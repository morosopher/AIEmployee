...
def test_brief_item_requires_source_and_safe_action() -> None:
    ...
    with pytest.raises(ValidationError):
        BriefItem(section="mail_summary", title="x", body_markdown="x", source_refs=[BriefSourceRef(source_type="mail", source_id="1")], suggested_action_kind="send_email")
...
async def test_graph_filters_spam_and_renders_sources() -> None:
    state = {"local_date": "2026-08-04", "source_cutoff": "2026-08-04T00:00:00Z", "model_gateway": FakeModelGateway(), "mail_threads": [{"thread_id": "spam", "sender": "x", "subject": "x", "labels": ["SPAM"]}, {"thread_id": "normal", "sender": "x", "subject": "hello"}], "calendar_events": []}
    result = await build_daily_brief_graph().ainvoke(state)
    assert all(item["source_refs"] for item in result["content"]["items"])
    assert "spam" not in {item["source_refs"][0]["source_id"] for item in result["content"]["items"]}
