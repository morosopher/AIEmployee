...
const latest = ref<Brief | null>(null); const versions = ref<Brief[]>([]); const selected = ref<Brief | null>(null); const loading = ref(true); const error = ref<string | null>(null); const taskId = vueRef<string | null>(null); useTaskEvents(taskId); const task = computed(() => taskId.value ? useTasksStore().tasks[taskId.value] : undefined); watch(() => task.value?.status, (status) => { if (status === 'succeeded') void load() })
