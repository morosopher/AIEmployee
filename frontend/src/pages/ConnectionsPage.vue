...
<template><section><h1>连接</h1><button type="button" @click="connect">连接 Google</button><p v-if="error" role="alert">{{ error }}</p><ul><li v-for="connection in connections" :key="connection.id"><strong>{{ connection.provider }}</strong> {{ connection.account_email }} · {{ connection.status }}<button type="button" @click="sync(connection)">立即同步</button><button type="button" @click="disconnect(connection)">断开</button></li></ul></section></template>
