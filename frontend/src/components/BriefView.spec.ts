...
describe('BriefView', () => {
  it('renders completeness, cutoff, warnings and source links', () => {
    const wrapper = mount(BriefView, {
      props: {
        brief: {
          ...
          warnings: ['Calendar unavailable'], items: [{ position: 1, section: '邮件', priority: 'high', title: '跟进', body_markdown: '内容', source_refs: [{ provider: 'gmail', object_id: 'm1', label: '邮件' }], suggested_action_kind: null }],
        },
      },
    })
    ...
    expect(wrapper.find('a').attributes('href')).toContain('gmail')
  })
})
