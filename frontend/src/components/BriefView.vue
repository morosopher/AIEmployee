...
<template>
  <article class="brief-view">
    ...
    <section v-for="item in brief.items" :key="item.position" class="brief-item"><h2>{{ item.title }}</h2><MarkdownMessage :content="item.body_markdown" /><div class="sources"><SourceLink v-for="source in item.source_refs" :key="`${source.source_type}:${source.source_id}`" :source="source" /></div><button v-if="item.suggested_action_kind && propose" type="button" @click="propose(item.suggested_action_kind)">创建任务建议</button></section>
  </article>
</template>
