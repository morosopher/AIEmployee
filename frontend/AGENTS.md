...
本文件适用于 `frontend/` 下的 Vue 应用、组件测试和 Playwright E2E，并继承根目录 `AGENTS.md`。本文件不得放宽根文件中的 M2 范围、继承自 M1 的可信执行不变量、安全、隐私、审批和真实性要求。
...
- M2 不显示或暗示可用的供应商草稿箱同步、转发、附件、HTML/富文本邮件、通讯录、批量操作、日程删除/取消、重复日程写入、会议链接、参会人 Free/Busy、完整 Outlook 替代品、完整收件箱或日历网格、文件上传、知识库、长期记忆、通用 Planner 或产品多 Agent 入口。
