...
revision-global lease 只解决 0019 窗口并发，不证明 OAuth refresh/exchange 的未知结果。所有既有 connection 的 writer——0019 preflight、Google/Microsoft mail 与 calendar Worker、渐进授权 callback 和 targetless callback——必须先通过供应商中立的 `OAuthRefreshCoordinator` 取得按 `connection_id` 隔离的 PostgreSQL session advisory lease；不得直接调用 `ensure_connection`、无条件 credential upsert 或让适配器自行 refresh/retry。source union 固定为 `calendar_aad_preflight`、`provider_refresh`、`progressive_callback`、`targetless_callback`。只有没有既有 connection、旧 credential 或 fence 的首次 bootstrap 可以在创建首行 connection 前交换授权码；它仍受一次性 OAuthAttempt/state 约束。

coordinator 的单次 attempt 必须按以下顺序运行：先在短事务中按 connection → access credential → refresh credential → matching started audit 的顺序锁定并冻结 generation、完整两行物理 snapshot、old `refresh_token_identity_v1` 与 identity key version；确认 claim 没有被其他 session 占用后，向既有 append-only `audit_events` 提交 content-free `calendar.aad_0019.refresh_started`，事务提交成功后才允许进入 provider call/exchange。网络阶段只保持 session lease，不持有业务事务；provider 前、响应后、CAS 前和结果提交前都必须由同一 session 证明 lease 仍在。响应后只能在另一个短事务中重新检查 lease、connection/capability/generation、两行旧 snapshot、old identity 和 attempt，再把 credential CAS、规范化结果事件与可选 replacement consumption 原子提交。

`refresh_started` 使用新生成的 canonical lowercase `refresh_attempt_id`，并绑定 `fence_schema_version="calendar_aad_0019_refresh_fence.v1"`、`source`、适用的 source/target revision 与 `rollout_digest_v1`、`connection_digest`、发起时冻结的 `fence_generation=F`、`pre_credential_snapshot_digest_v1`、`pre_refresh_credential_snapshot_digest_v1`、old `refresh_token_identity_v1`、`refresh_token_identity_key_version` 和稳定 `result_code`。provider 返回后，只有旧 snapshot/generation CAS、新 access/可选 refresh credential、精确 expiry 与复用相同 attempt/source/rollout/fence generation/两个 pre-digest/old identity-key-version，并强制增加 `post_credential_snapshot_digest_v1`、`post_refresh_credential_snapshot_digest_v1`、从同一事务实际持久化值读取的 `token_expires_at` 及其 `rollout_deadline_candidate = token_expires_at - 900 seconds` 的 matching result 同事务提交，才算完成。0019 preflight 写 `refresh_confirmed`；其他来源按关闭 schema 写普通结果及在满足条件时写 `oauth.refresh_credential_replaced`。

started 提交后的网络断开、provider 响应未知、响应后任一 lease 丢失、CAS miss、commit unknown、malformed token/expiry/scope、`invalid_grant` 或 scope shrink 都留下 started-without-confirmed，或追加绑定同一 attempt 的稳定 needs-attention/terminal 结果。相同 attempt 的 Taskiq 重投、`TransientProviderError`、进程恢复或 callback 重入只能读取持久 fence 并返回 `oauth_refresh_result_unknown`/`oauth_refresh_claim_lost` 等稳定 non-retryable 结果，`provider_calls == 0`；不得把普通 retry 次数当作再次调用供应商的授权。只有 provider call 尚未开始且 started 尚未提交的纯本地失败才可安全重试 claim。若 confirmed 已提交但 artifact 发布前崩溃，同一 rollout 必须从持久新 credential/expiry 继续 probe 和发布 artifact，provider refresh 调用数保持不变。

identity 与摘要协议不得由脚本或实现自行解释。`refresh_token_identity_v1` 对 non-empty refresh plaintext 使用密钥化 fingerprint：从版本化应用主密钥以 HKDF-SHA256 和 M2 规格 17.3 固定 salt/info 派生 32-byte fingerprint key，再对 domain-separated framed plaintext 做 HMAC-SHA256。事件同时持久化 identity 与 `refresh_token_identity_key_version`，不持久化 token、应用主密钥或派生 HMAC key。新 attempt 使用活动 key version；既有事件不得被静默改写到新版本，旧 fingerprint key 必须保留到所有引用它的 unresolved fence、consumption、rollback guard 和 retention 组完成生命周期与 cutoff。轮换前必须先证明旧版本仍可用，轮换后以旧事件记录的版本重算 current identity；缺 key 一律 fail closed。任何日志、Trace、SSE、API、fixture 或 release evidence 都不得输出真实 refresh plaintext、应用主密钥、派生 key 或可用于关联真实凭据的 identity；发布证据只记录固定 synthetic vector 的通过/失败和版本策略。

`credential_snapshot_digest_v1` 是 SHA-256 小写十六进制，规范字节以 `b"AIEMPLOYEE/calendar-aad/credential-snapshot/v1\x00"` 开头，顶层固定为 access row、refresh row；`authorization_generation` 不进入摘要，而是 fence/consumption 的独立字段。`refresh_credential_snapshot_digest_v1` 使用 domain `b"AIEMPLOYEE/calendar-aad/refresh-credential-snapshot/v1\x00"`，只编码 refresh row。每行固定为 kind、row ID、user ID、connection ID、ciphertext、nonce、key version、expiry、updated-at。`rollout_digest_v1` 使用 domain `b"AIEMPLOYEE/calendar-aad/rollout/v1\x00"`，固定绑定 preflight schema、source/target revision、安全 basename、精确小写 `sha256:...` image ID 和 `900` 秒余量。三个协议的每个字段都以 `NULL => 0x00`、非 NULL `=> 0x01 || uint32_be(length) || raw_bytes` framing；UUID 为小写 canonical ASCII，整数为规范十进制 ASCII，时间为 UTC RFC3339 固定六位 microseconds 加 `Z`，ciphertext/nonce 使用原始 bytes。空 bytes 不等于 NULL。字段、顺序、domain 或编码变化必须新增版本；不得把 raw scope/calendar ID 放入摘要，或让 started/confirmed/terminal/consumption/retention 使用不同 helper。两个 credential 摘要都只证明数据库物理 snapshot/CAS；ciphertext、nonce、key version、`updated_at` 或摘要变化均不能证明 refresh token plaintext 已替换，也不能承担 rollback 判定。四个完整固定合成向量以 M2 规格 17.3 为准，Task 27 单测的 expected bytes/base64/hash/HMAC 必须是独立常量。

`oauth.refresh_credential_replaced` 是某个 unresolved `refresh_started` 的 append-only、不可逆 terminal/consumption fact，不是要求提交后状态与后续状态永久相等的 lineage marker。首次成功消费必须锁定并绑定原 fence 的 `refresh_attempt_id`、`started_source`、`fence_generation=F`、`old_refresh_credential_snapshot_digest_v1`、old `refresh_token_identity_v1`、old identity key version、来源与来源代际；供应商必须明确返回 non-empty 新 refresh token，并在受控内存中以 `secrets.compare_digest(old, new)` 证明 plaintext 不同。只有旧 refresh snapshot/identity 与 connection generation 的精确 CAS 成功、写入新 refresh row 并在同一事务追加包含 new identity/key version 的 consumption event，原 fence 才永久 resolved。之后正常 authorization generation、capability/scope、access token 或 refresh token 变化都不会使旧 fence 复活，也不要求后续 generation/digest 保持 event 提交时的值。若 scope 后来进入 `action_required`，后续授权走普通 existing-connection 代际协议，不再绑定旧 fence。preflight 只验证 matching consumption 的 schema、原 attempt/F/old physical digest/old identity/key version 与来源代际不变量，再独立验证当前 connected connection、能力、scopes 和 credential；若要建立新 attempt，必须从通过校验的 current generation、两行 snapshot 和 current identity 重新冻结新 `F`、两个 pre-digest 与 identity key version，不得沿用已消费 fence 的旧 pre-state。rollback guard 必须按 consumption 记录的 old key version 在受控内存重算 current plaintext identity；current identity 等于 consumed old identity 时，即使 physical digest 已变化（包括 A→B→A 或同 token 重加密），也以 `oauth_refresh_identity_rollback` fail closed。physical digest 只用于配对和 CAS，不能作为 rollback 判断。

consumption metadata 使用关闭的 source-tagged union，共同绑定 `proof_schema_version="oauth_refresh_credential_replaced.v1"`、`source`、`started_source`、`connection_digest`、原 `refresh_attempt_id`、`fence_generation=F`、old/new refresh physical digest、old/new `refresh_token_identity_v1` 与各自 key version、`source_generation`、`pre_generation`、`post_generation` 和稳定 `result_code`。`source="progressive_callback"` 时，仍连接的原 connection 按首次恢复协议从 `source_generation=S` 发起，既有 `set_capabilities_authorizing` 只递增一次到 `target_generation=T=S+1`，`OAuthAttempt` 只保存 target `T`，callback anti-replay 后保持 `pre_generation=post_generation=T`；`F <= S` 且发起时 current identity 必须按 started key version 计算后等于 fence old identity，physical digest 仅同时满足旧 snapshot CAS。`source="targetless_callback"` 时，OAuthAttempt 没有 target `T`；start 必须先冻结 candidate existing connection，callback 在 exchange 前取得该 connection coordinator lease，规范化 provider identity 必须命中同一 connected row，并保持满足 `F <= G` 的 `source_generation=pre_generation=post_generation=G`，不得递增 generation 或创建第二个 connection。`source="provider_refresh"` 时，普通 Google/Microsoft mail/calendar refresh 同样在读取代际 `G` 下保持 G/G/G。三个来源都必须 compare-digest、identity/snapshot/generation CAS，并把 credential 与 consumption 原子提交；锁住 started 后重新检查未消费状态，确保竞争 writer 最多只有一个成为 terminal consumer。

所有既有 connection credential rotation writer 都必须复用同一 coordinator 与 snapshot-CAS boundary：Google/Microsoft mail/calendar resolver、渐进 callback、targetless callback 和 0019 主动 preflight 都携带读取时 connection generation，以及 access/refresh 两行的 row ID、用户/连接/kind、`updated_at`、完整 AEAD、expiry、old identity 与 key version。短事务统一按 connection → access row → refresh row → matching started audit 的锁序重检；CAS miss 不覆盖任何 credential，也不消费 fence。provider 未返回新 refresh token，或返回与旧 plaintext 相同的值时，只允许 CAS 更新 access row并逐字节保留 refresh row/old identity，不生成 consumption。普通 Worker 在满足明确的新 attempt/recovery 协议并取得确定不同 refresh token时可用 `source="provider_refresh"` 消费；targetless callback 只能合并已冻结且 identity 匹配的既有 connected row，禁止 `ensure_connection`、无条件 upsert 或并发创建重复 connection。网络未知、响应无/同 refresh、事务失败、响应后 lock loss 或 stale CAS 都继续保留 unresolved fence；同一 attempt 的任何 Worker/Taskiq/callback 重入必须 zero provider calls。0019 preflight 对自己刚提交的 started 只写 matching `refresh_confirmed`，不把同一 attempt 标记为 consumption。

365 天审计清理对 refresh fence 有强制例外：unresolved started 没有 matching confirmed、可证明安全关闭的 known-terminal 或有效 terminal consumption 时必须跨 cutoff 保留。retention 只能在显式 `user_id` 范围内解析 content-free metadata；matching `oauth.refresh_credential_replaced` 只需作为该 started 的有效 terminal consumption，精确匹配原 attempt/F/old refresh physical digest/old identity/key version、`started_source` 并满足对应 source generation 不变量，不再读取或约束后续 current credential/generation lineage。通用 audit delete 必须排除 started/confirmed/terminal 与 replacement consumption；专用清理只能在同一事务按 attempt 成组删除已证明安全的 started 及其 matching terminal result。started 与 consumption/result 每一行都必须早于 cutoff，等价要求组内最大 `created_at < cutoff`；旧 started 匹配较新的 result 时不得提前删除，也不能先删 result 再留下看似未决的 started。清理与所有 credential CAS 统一采用 connection → access credential → refresh credential → started audit 的锁序，锁后重查 matching result；另一用户与普通过期 audit 必须继续按正常 cutoff 处理。仍被 unresolved/consumed/rollback/retention 组引用的 identity key version 不得提前移除；无需为了 retention 保留消费后的后续 credential lineage。
...
3. 运行无参数 `just calendar-aad-preflight-0019`。该 0018-compatible one-off 先取得上述 revision-global lease，再检查 revision、artifact 不存在/不冲突并从历史完整 AEAD 三元组推导精确 affected pairs。它按 connection UUID 去重并串行处理每个 connection；在任何 refresh provider call 前还必须通过共享 `OAuthRefreshCoordinator` 取得该 connection 的 session lease，使 revision-global 与 connection-scoped lease 同时有效。随后冻结 access/refresh 两行的 `id`、用户/连接/kind、完整 AEAD、`token_expires_at` 和 `updated_at`，以唯一 helper 计算 current full/refresh credential digest 和 `refresh_token_identity_v1`/key version，并重检 authorization generation 与 refresh fence。若同一 rollout/attempt 已有与当前持久两个 post-digest、持久 expiry 及其 deadline candidate 匹配的 `refresh_confirmed`，则直接恢复后续 probe，禁止再次 refresh；若存在未确认/needs-attention fence，则先查找 matching `oauth.refresh_credential_replaced` consumption。consumption 必须精确匹配原 `refresh_attempt_id`、`started_source`、原 fence generation、old refresh physical digest/identity/key version，并满足 `progressive_callback`、`targetless_callback` 或 `provider_refresh` 的代际不变量；它一经有效即永久关闭旧 fence，不要求 event 提交后的 generation/refresh digest 在后续保持不变。preflight 再按 consumption 记录的 old key version 重算 current plaintext identity；若 current identity 回到 consumed old identity，即使 physical digest 不同，也以 credential rollback fail closed。否则重新独立验证 current connection 仍 connected、`calendar.read` enabled、scopes 完整且 access/refresh credential 存在、归属正确并可解密，才能建立新 attempt。新 attempt 必须使用本轮 current generation、两行 snapshot 与 current identity/key version 重新冻结 `fence_generation=F`、两个 pre-digest 和 old identity，不得复用已消费 fence 的旧 pre-state。没有 matching consumption 时，同名或不同 basename 都以零 provider call 失败。它先完成 access/refresh 存在性、归属和 AEAD 可解密性等本地校验；通过后再次核验两层 lease，在紧邻 provider 调用前提交绑定 `source="calendar_aad_preflight"`、attempt UUID、source/target revision、`rollout_digest_v1`、`fence_generation=F`、两个 pre-digest、old identity 与 key version 的 `refresh_started`，再使用已解密的 refresh credential 主动调用现有 Google/Microsoft OAuth refresh。access-only、缺失/不可解密 refresh 必须在 started/provider 前 fail closed；refresh 被拒绝或返回 malformed access/expiry/scope 则留下 durable fence，并在任何 `initial_pages` 前 fail closed。返回的 access token 必须非空且可用，`granted_scopes` 必须覆盖该 connection 保存的 canonical scopes 并包含 `calendar.read` 所需 provider scope。验证后只能通过所有 writer 共享的 coordinator/snapshot-CAS repository，在短事务中精确匹配 connection generation、old identity 与两行旧 snapshot，并重检两层 lease、owning connection、fence generation 和 `calendar.read` 能力，再把 credential rotation、实际持久化 expiry 与绑定同一 attempt/source/fence generation/两个 pre-digest/old identity-key-version、两个新 post-digest 及 `rollout_deadline_candidate = token_expires_at - 900 seconds` 的 `refresh_confirmed` 原子提交；响应没有新 refresh token，或新值与旧 plaintext 相同时，仍校验旧 refresh row snapshot 并逐字节保留。现有无条件 credential upsert 禁止用于该路径。任一未知结果、响应后任一 lease 丢失、CAS miss 或 commit unknown 都不覆盖并发新 token、不执行后续 probe、不发布 artifact，revision 保持 0018；后续 Taskiq delivery 或 `TransientProviderError` 路径只读 fence，返回稳定 non-retryable unknown 且 provider call 为零。每个 confirmed rotation 提交前先证明该 token 的 expiry 晚于 current UTC 加 900 秒；全部 connection 已 confirmed 后以 confirmed 中持久化 deadline candidate 的最小值计算全局 `rollout_deadline`，再按确定性 pair 顺序调用与迁移后恢复相同的 `initial_pages(scope_key)`，要求最终 cursor。不等待资源 401，主动刷新后的资源 401、403、权限不足、malformed page 或无最终 cursor 都直接失败。它不得调用 `directory_pages()`、connection-level/full-account sync、其他 pair 或 Calendar 写适配器，也不得写 CalendarEvent、cursor 或 marker；仅允许上述 fence 与 credential CAS/confirmed 事实。非空 set 在每个 pair probe 前及 rollout artifact 原子提交前检查 `now < rollout_deadline`，zero branch 则在 artifact 提交前重新证明 affected set 仍为空。任何失败都返回非零并停止变更窗口。
...
以下命令从 `/srv/ai-employee` 运行，不包含账号、DSN、正文、原始 `calendar_id` 或凭据。migration one-off 继续从 Compose Secret 读取数据库凭据。`BACKUP_ARTIFACT_BASENAME`、content-free preflight state、revision-global lease、共享 `OAuthRefreshCoordinator` connection lease、dual credential digest、版本化 refresh identity、source-tagged refresh replacement consumption、共享 credential snapshot CAS、`just calendar-aad-preflight-0019`、`just calendar-aad-migrate-0019`、`just calendar-aad-audit phase artifact`、`just calendar-aad-resync-0019`、专用 owner-role restore service、`just calendar-aad-verify-restored-0018` 及其合成数据库测试属于 Task 27 要落地的运维契约；当前 0019 recipe/CLI、coordinator 与 owner restore 边界尚未实现。现有 `just restore` 复用 app-role `backup` 服务，不能执行受控整库 DDL restore，也不得被当作 sealed-window 恢复证据。未完成 Task 27 前不得执行生产 0019 或整库恢复，也不得用临时命令、额外 DDL 授权或直接 SQL 替代。示例镜像标签必须替换为预先构建并固定的实际不可变标签，不得使用 `latest`。
...
preflight 的供应商表面严格只读，但 refresh 本身是可能轮换凭据的外部写效应，因此必须使用 durable fence 和共享 coordinator。`refresh_started`、`refresh_confirmed` 和 `calendar.aad_0019.refresh_terminal` 使用既有 `audit_events`、`task_id=NULL`、`actor_type=system`；started/terminal metadata 只含关闭 schema 允许的 `source`、canonical lowercase `refresh_attempt_id`、source/target revision、`rollout_digest_v1`、`connection_digest`、`fence_generation`、对应 pre `credential_snapshot_digest_v1`/`refresh_credential_snapshot_digest_v1`、old `refresh_token_identity_v1`/key version 和稳定 `result_code`；confirmed 必须额外包含两个 post digest、实际持久化 `token_expires_at` 及由它减去 900 秒得到的 `rollout_deadline_candidate`。本地 credential 存在性、归属和可解密性校验必须在 started 前完成；revision-global 与 connection coordinator lease 都必须已取得，紧邻 provider 的 started 事务与 provider 后的 credential CAS/confirmed 事务都很短，网络阶段不持有业务事务。已 confirmed 且 current credential 与事件的两个 post-digest 匹配时，同一 rollout 从持久 expiry 继续，不再次 refresh；started-without-confirmed 或 needs-attention 对所有 basename 都是硬阻断。matching refresh replacement consumption 必须验证原 attempt/F/old physical digest/old identity/key version、`started_source` 与来源代际不变量，并永久关闭原 fence；preflight 随后独立验证 current connection/capability/scopes/credential，且按事件 key version 重算的 current identity 回到 consumed old identity 时必须以 rollback fail closed。普通 access-only rotation、generation、物理摘要变化和显式人工处置都不得成为 replay waiver；普通 Worker 或 callback 只有按共享协议确定提交不同 refresh plaintext 时，才可通过 `provider_refresh`、`progressive_callback` 或 `targetless_callback` 成为 terminal consumer。

snapshot 对 access/refresh 每行都绑定 `id`、用户/连接/kind、ciphertext、nonce、key version、`token_expires_at` 与 `updated_at`，并另行绑定 old refresh identity/key version；共享短事务还要重检 connection/capability/generation，并按 connection → access row → refresh row → matching started audit 锁序执行。响应省略新 refresh token 或返回相同 plaintext 时只更新 access row，精确匹配并逐字节保留旧 refresh 行；任一 CAS miss 都不会覆盖新 token、消费 fence 或发布 artifact。网络断开、任一 lease 丢失、响应后 commit unknown、malformed response、`invalid_grant` 与 scope shrink 都不得对同 snapshot 自动重试；同 attempt 的 Taskiq/`TransientProviderError` 再进入时 provider call 必须为零。它不等待资源 401，也不在 probe 阶段再刷新；主动刷新后的资源 401、403 或 scope 缺失均是失败。除 refresh fence 与成功的 credential CAS/confirmed 事实外，不得写连接、能力或其他业务事实，也不得写 CalendarEvent、SyncCursor 或 marker。revision-global lease 与当前 connection coordinator lease 必须在每个 provider call 和 credential CAS 前仍可由各自同一 session 证明持有，artifact publish 前还必须重检 revision-global lease 和所有 connection result 已稳定确认。每个 confirmed access token 的持久 `token_expires_at` 参与固定 `rollout_deadline = min(token_expires_at) - 900 seconds`；preflight state artifact 只保存 schema/revision、`rollout_digest_v1`、安全 basename、宿主机解析的 immutable image content ID、earliest expiry/deadline、固定 margin、affected/connection 计数、lowercase pair digest、hashed connection IDs 和稳定结果码，不保存 token、scope 原文、正文、raw `calendar_id`、HMAC key 或真实 credential identity。preflight、backup、migration、resync、audit 每个 CLI/recipe 都必须在启动和关键本地提交前读取并校验该 artifact。非空 affected set 的任何 `now >= rollout_deadline` 都 fail closed；zero/no-deadline state 只有在规范零值、镜像/basename 绑定有效且重新查询仍无 affected pair 时才通过，否则同样 fail closed。

任一 pair 或 connection 失败时不得继续备份、审计或 migration；数据库仍为 0018。若存在未确认/needs-attention refresh fence，后续同名或不同 basename invocation 都不得再次调用 preflight provider，同一 attempt 的 Worker、Taskiq、`TransientProviderError` 或 callback 重入也必须保持 zero provider calls。保持写开关关闭；原 connection 仍为 `connected` 时，只能通过共享 coordinator 下的明确新恢复流程取得并原子提交真正不同的 non-empty refresh token：渐进重授权/重新同意记录 `source="progressive_callback"` 与 F/S/T；没有 target generation、但 start 已冻结 candidate existing connection 且 callback provider identity 命中同一 row 时记录 `source="targetless_callback"` 与满足 `F <= G` 的 G/G/G；正常 Google/Microsoft mail/calendar Worker refresh 记录 `source="provider_refresh"` 与 G/G/G。三者都必须绑定 matching started 的 old physical digest/identity/key version、compare-digest、完整 snapshot/generation CAS 和单消费者 event append；不得为消费 fence 启动临时 OAuth-only 服务、强制资源 401 或额外 provider 调用。

targetless callback 的事故处置必须保留原 connection 和 credential：identity mismatch、candidate row 已断开、并发 callback 已消费、CAS miss、响应后 lock loss 或 exchange 结果未知时，禁止 `ensure_connection`、无条件 upsert、创建第二个 connection 或复用同一 code/attempt 再 exchange；记录稳定错误并转 `needs_attention`。恢复先用只读查询核对 OAuthAttempt、candidate snapshot、provider identity mapping、coordinator lease/fence 与是否已有 matching terminal fact；如果同一 attempt 已 started 且结果未知，任何自动或人工“重试”都只读 fence。只有新的、明确用户发起且满足既有代际/claim 协议的授权尝试，或已经由另一受控 writer 确定提交的新 identity 并能无网络完成 matching consumption CAS，才可能解除阻断；身份或状态无法证明时保持 fail closed。

**发现未知 fence 时不要先 disconnect**：断开会先删除旧 credential，之后无法比较 old/new plaintext；断开后重连、新建 connection 或映射新 token 都不能生成或替代 consumption。callback/Worker 未返回 refresh token、返回相同 plaintext、旧 plaintext 不可得、connection 已断开，或仅改变 access/generation/物理密文时仍保持阻断。若 current plaintext identity 按 consumed event 的 old key version 重算后回到 old identity，包括 A→B→A 或同 token 重加密，即使 physical digest 不同也必须以 `oauth_refresh_identity_rollback` fail closed。若断开后 affected scope 仍存在，只能走既有明确用户授权的数据处置/删除历史敏感字段流程并重新评估 affected set；若该流程也不适用，必须停止并请求新的已批准设计。禁止 fabricated consumption、新连接映射、marker skip、retry waiver、直接 SQL、伪造 credential、access-only 迁移或先迁移再清 marker。显式人工调查结果不能授权同 snapshot 重放；除非另有独立持久的不可调用终态，否则也不能让 retention 删除唯一 started 防线。已 confirmed 但尚未发布 artifact 的崩溃恢复必须复用同一 rollout 与持久新 token。其他本地事实失败可在恢复精确目录事实后重新开启完整变更窗口。
...
Task 30 release evidence 还必须记录共享 coordinator 的 content-free 证明：所有五类调用方都经过同一 application port；0019 每个 connection 在 provider 前同时持有 revision-global 与 connection-scoped lease；started 已在网络前提交；provider 后 credential CAS/result/consumption 原子提交；两个 Worker 竞争时仅一次 provider call；响应后 connection lease 丢失、网络 unknown、CAS miss、commit unknown、Taskiq 重投和 `TransientProviderError` 后续调用均为零。OAuth 证据必须覆盖 progressive F/S/T、targetless `F <= G` 的 G/G/G 且没有重复 connection、provider-refresh G/G/G、same/empty/no refresh 不消费、A→B→A 与同 token 重加密 identity rollback，以及 identity key rotation/旧版本保留。四个固定 synthetic vector 必须独立重算并记录 pass/fail。证据只能包含 attempt/source、稳定结果码、hashed connection、generation tuple、CAS/lease/provider-call 计数和 synthetic vector 标识；不得包含真实 token、identity fingerprint、主密钥、派生 HMAC key、raw scope 或 provider response。
...
出现 `oauth_refresh_result_unknown`、`oauth_refresh_claim_lost`、`oauth_refresh_identity_rollback`、同一 attempt 第二次 provider-call 企图、targetless identity mismatch/重复 connection 企图，或缺失 identity key version 时，立即关闭受影响 provider/connection 的新写入与会触发 refresh 的同步入口；0019 窗口则继续保持全部通用服务停止。不要 disconnect、重放 Taskiq、复用 OAuth code、手工改 credential、删除 audit fence 或调用供应商“探测”。先以只读方式收集 connection digest、attempt/source、generation tuple、lease/CAS result、started/terminal/consumption event 和 provider-call count；日志与工单不得包含 token、scope 原文、identity fingerprint 或密钥材料。

若 matching confirmed/result 已持久化，只从持久 credential/result 恢复后续本地工作，provider call 保持不变；若 started 后结果未知、lease 丢失或 CAS/commit 不可证明，则保持 `needs_attention` 和 zero-call replay，直到已有受控 writer 能无网络提交 matching terminal fact，或新的明确用户授权流程满足 coordinator 的 generation/identity/CAS 协议。targetless 事故必须证明 candidate existing row、normalized provider identity 和 G/G/G tuple 完全一致，无法证明时不得 merge 或新建 connection。identity rollback 事故按 event 记录的 old key version 重算 current identity；A→B→A、同 token 重加密或缺旧 key 都保持 fail closed。只有 content-free 审计证明 fence 已安全 terminal、重复 delivery 为零调用、连接能力与 scopes 重新有效后，才能恢复该连接；0019 还必须重新开始完整 preflight/deadline/backup 窗口。
