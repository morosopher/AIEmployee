...
lease 只解决并发，不证明 OAuth refresh 的未知结果。每个 connection 在调用 provider 前，还必须在短事务内向既有 append-only `audit_events` 追加 content-free `calendar.aad_0019.refresh_started` fence；不新增 0018 Schema。事件使用新生成的 canonical lowercase `refresh_attempt_id`，并绑定 `fence_schema_version="calendar_aad_0019_refresh_fence.v1"`、`rollout_digest_v1`、`connection_digest`、当前 `authorization_generation`、`pre_credential_snapshot_digest_v1`、`pre_refresh_credential_snapshot_digest_v1` 和 `result_code`，但不保存 token、raw scope、响应、正文或 raw `calendar_id`。provider 返回后，只有旧 snapshot CAS、新 access/可选 refresh credential、精确 expiry 与复用相同 attempt/rollout/generation/两个 pre-digest、增加 `post_credential_snapshot_digest_v1` 和 `post_refresh_credential_snapshot_digest_v1` 的 `calendar.aad_0019.refresh_confirmed` 在同一事务提交，才算 refresh 完成。网络断开、调用后 lease 丢失、响应后 CAS/事务失败、malformed token/expiry/scope、`invalid_grant` 或 scope shrink 都留下 started-without-confirmed，或追加绑定同一 attempt 的稳定 needs-attention/`calendar.aad_0019.refresh_terminal` 结果；任何同名或不同 basename 的后续 preflight 都不得再次调用 provider。只有精确匹配的 `oauth.refresh_credential_replaced` 证明事件才能自动解除未知 fence；普通 access-token rotation、generation-only、任意物理摘要变化或显式人工处置都不能允许重放。若 confirmed 已提交但 artifact 发布前崩溃，同一 rollout 必须从持久新 credential/expiry 继续 probe 和发布 artifact，provider refresh 调用数保持不变。

摘要协议不得由脚本或实现自行解释。`credential_snapshot_digest_v1` 是 SHA-256 小写十六进制，规范字节以 `b"AIEMPLOYEE/calendar-aad/credential-snapshot/v1\x00"` 开头，顶层固定为 access row、refresh row；`authorization_generation` 不进入摘要，而是 fence/proof 的独立字段。`refresh_credential_snapshot_digest_v1` 使用 domain `b"AIEMPLOYEE/calendar-aad/refresh-credential-snapshot/v1\x00"`，只编码 refresh row。每行固定为 kind、row ID、user ID、connection ID、ciphertext、nonce、key version、expiry、updated-at。`rollout_digest_v1` 使用 domain `b"AIEMPLOYEE/calendar-aad/rollout/v1\x00"`，固定绑定 preflight schema、source/target revision、安全 basename、精确小写 `sha256:...` image ID 和 `900` 秒余量。三个协议的每个字段都以 `NULL => 0x00`、非 NULL `=> 0x01 || uint32_be(length) || raw_bytes` framing；UUID 为小写 canonical ASCII，整数为规范十进制 ASCII，时间为 UTC RFC3339 固定六位 microseconds 加 `Z`，ciphertext/nonce 使用原始 bytes。空 bytes 不等于 NULL。字段、顺序、domain 或编码变化必须新增版本；不得把 raw scope/calendar ID 放入摘要，或让 started/confirmed/terminal/retention 使用不同 helper。两个 credential 摘要都只证明数据库物理快照；ciphertext、nonce、key version、`updated_at` 或摘要变化均不能证明 refresh token 明文已替换。完整固定合成向量以 M2 规格 17.3 为准，Task 27 单测的 expected bytes/base64/hash 必须是独立常量。

唯一自动解锁证明是 append-only content-free `oauth.refresh_credential_replaced`。Google/Microsoft 渐进授权 callback 必须在短受控内存中解密旧 refresh token；仅当供应商明确返回 non-empty 新 refresh token，且对规范 bytes 执行 `secrets.compare_digest(old, new)` 为 false 时，才可在一个短事务内重检 old generation 与 old refresh digest、写入新 refresh credential、递增 generation，并追加证明事件。proof 绑定 `proof_schema_version="oauth_refresh_credential_replaced.v1"`、`connection_digest`、`old_refresh_credential_snapshot_digest_v1`、`new_refresh_credential_snapshot_digest_v1`、`old_authorization_generation`、`new_authorization_generation` 和稳定 `result_code`，用户由 AuditEvent 的非空 `user_id` 绑定；不得保存 token、plaintext hash、raw scope 或 provider response。callback 没有 refresh token、返回空值、返回相同 plaintext、仅更新 access、只改变 generation、相同明文重加密或普通 token refresh 均不得生成 proof 或解锁。provider 已返回新 token 但进程在本地事务提交前崩溃时，新 credential、generation 与 proof 必须全部不可见，旧未知 fence 继续阻断。

365 天审计清理对 refresh fence 有强制例外：unresolved started 没有匹配 confirmed 或可证明安全关闭的 known-terminal 时必须跨 cutoff 保留。retention 只能在显式 `user_id` 范围内解析 content-free metadata；旧 fence 只允许通过 matching confirmed/安全 terminal、old generation/refresh digest 精确匹配 started 且当前 generation/refresh digest 精确匹配 proof new 值的 `oauth.refresh_credential_replaced`，或可独立证明的永久 disconnected/数据处置终态解除。任意 full/refresh physical digest、generation、ciphertext、nonce、key version 或 `updated_at` 变化都不能单独清理或解锁；known-terminal 若清理后会失去唯一重放防线，也继续按 unresolved 处理。通用 audit delete 必须排除 started/confirmed/terminal 与 `oauth.refresh_credential_replaced`；专用清理只能在同一事务按 attempt 成组删除已证明安全的 started，以及用于证明安全的 matching result 和存在时的 replacement proof。组内每个待删事件都必须早于 cutoff，等价要求最大 `created_at < cutoff`。旧 started 匹配较新的 result/proof 时不得提前删除，也不能先删结果再留下看似未决的 started。清理与 confirmed 事务统一采用 connection → access credential → refresh credential → started audit 的锁序，锁后重算两个 credential digest、重查 matching result/proof 与当前 generation；另一用户与普通过期 audit 必须继续按正常 cutoff 处理。
...
preflight 的供应商表面严格只读，但 refresh 本身是可能轮换凭据的外部写效应，因此必须使用 durable fence。`refresh_started`、`refresh_confirmed` 和 `calendar.aad_0019.refresh_terminal` 使用既有 `audit_events`、`task_id=NULL`、`actor_type=system`；metadata 只含 `fence_schema_version`、canonical lowercase `refresh_attempt_id`、`rollout_digest_v1`、`connection_digest`、`authorization_generation`、pre/post `credential_snapshot_digest_v1`、pre/post `refresh_credential_snapshot_digest_v1`、明确允许的 `token_expires_at` 和 `result_code`。本地 credential 存在性、归属和可解密性校验必须在 started 前完成；紧邻 provider 的 started 事务与 provider 后的 credential CAS/confirmed 事务都很短，网络阶段不持有业务事务。已 confirmed 且当前 credential 与事件的两个 post-digest 匹配时，同一 rollout 从持久 expiry 继续，不再次 refresh；started-without-confirmed 或 needs-attention 对所有 basename 都是硬阻断。只有 matching replacement proof 的 old generation/refresh digest 匹配 started 且当前状态匹配 proof new 值时，才可建立新 fence；普通 access-token rotation、generation 或物理摘要变化和显式人工处置都不得成为 replay waiver。
...
任一 pair 或 connection 失败时不得继续备份、审计或 migration；数据库仍为 0018。若存在未确认/needs-attention refresh fence，后续同名或不同 basename invocation 都不得再次调用 provider；保持写开关关闭，并要求用户通过既有 OAuth 流程取得真正不同的 non-empty refresh token，使 callback 原子提交 matching `oauth.refresh_credential_replaced` proof，才能开启新尝试。callback 未返回 refresh token、返回相同 plaintext 或仅改变 access/generation/物理密文时仍保持阻断；用户也可断开并重新连接取得真正新 token，或走既有永久数据处置流程。显式人工处置只能记录绑定原 attempt 的终止/调查结果，不允许同 snapshot 重放；除非连接已有独立持久的不可调用终态，否则该结果也不能让 retention 删除唯一 started 防线。已 confirmed 但尚未发布 artifact 的崩溃恢复必须复用同一 rollout 与持久新 token。其他本地事实失败可在恢复精确目录事实后重新开启完整变更窗口。若数据只能按既有明确用户授权的处置流程删除，应走该流程并重新评估 affected set；禁止 retry waiver、直接 SQL、伪造 credential、access-only 迁移或先迁移再清 marker。
