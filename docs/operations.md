...
lease 只解决并发，不证明 OAuth refresh 的未知结果。每个 connection 在调用 provider 前，还必须在短事务内向既有 append-only `audit_events` 追加 content-free `calendar.aad_0019.refresh_started` fence；不新增 0018 Schema。事件使用新生成的 canonical lowercase `refresh_attempt_id`，并绑定 `fence_schema_version="calendar_aad_0019_refresh_fence.v1"`、`rollout_digest_v1`、`connection_digest`、当前 `authorization_generation`、唯一 `pre_credential_snapshot_digest_v1` 和 `result_code`，但不保存 token、raw scope、响应、正文或 raw `calendar_id`。provider 返回后，只有旧 snapshot CAS、新 access/可选 refresh credential、精确 expiry 与复用相同 attempt/rollout/generation/pre-digest、增加 `post_credential_snapshot_digest_v1` 的 `calendar.aad_0019.refresh_confirmed` 在同一事务提交，才算 refresh 完成。网络断开、调用后 lease 丢失、响应后 CAS/事务失败、malformed token/expiry/scope、`invalid_grant` 或 scope shrink 都留下 started-without-confirmed，或追加绑定同一 attempt 的稳定 needs-attention/`calendar.aad_0019.refresh_terminal` 结果；任何同名或不同 basename 的后续 preflight 都不得再次调用 provider。只有既有 OAuth 流程同时递增授权代际并改变当前 `credential_snapshot_digest_v1`，才能开启新尝试；普通 access-token rotation 不能绕过 fence，显式人工处置也只能记录终止/调查结果，不能允许同 snapshot 重放。若 confirmed 已提交但 artifact 发布前崩溃，同一 rollout 必须从持久新 credential/expiry 继续 probe 和发布 artifact，provider refresh 调用数保持不变。

摘要协议不得由脚本或实现自行解释。`credential_snapshot_digest_v1` 是 SHA-256 小写十六进制，规范字节以 `b"AIEMPLOYEE/calendar-aad/credential-snapshot/v1\x00"` 开头；顶层固定为 authorization generation、access row、refresh row，每行固定为 kind、row ID、user ID、connection ID、ciphertext、nonce、key version、expiry、updated-at。`rollout_digest_v1` 使用 domain `b"AIEMPLOYEE/calendar-aad/rollout/v1\x00"`，固定绑定 preflight schema、source/target revision、安全 basename、精确小写 `sha256:...` image ID 和 `900` 秒余量。两个协议的每个字段都以 `NULL => 0x00`、非 NULL `=> 0x01 || uint32_be(length) || raw_bytes` framing；UUID 为小写 canonical ASCII，整数为规范十进制 ASCII，时间为 UTC RFC3339 固定六位 microseconds 加 `Z`，ciphertext/nonce 使用原始 bytes。空 bytes 不等于 NULL。字段、顺序、domain 或编码变化必须新增版本；不得恢复双摘要、把 raw scope/calendar ID 放入摘要，或让 started/confirmed/terminal/retention 使用不同 helper。完整固定合成向量以 M2 规格 17.3 为准，Task 27 单测的 expected bytes/base64/hash 必须是独立常量。

365 天审计清理对 refresh fence 有强制例外：unresolved started 没有匹配 confirmed 或可证明安全关闭的 known-terminal 时必须跨 cutoff 保留。retention 只能在显式 `user_id` 范围内解析 content-free metadata；只有 matching result、既有 OAuth 重新授权同时改变 generation 与 `credential_snapshot_digest_v1`，或连接进入可独立证明的永久 disconnected/数据处置终态时，旧 fence 才可清理。generation 或 digest 仅变化一个、尤其普通 access-token rotation，均不得清理或解锁。known-terminal 若清理后会失去唯一重放防线，也继续按 unresolved 处理。通用 audit delete 必须排除 started/confirmed/terminal；专用清理按 attempt 在同一事务成组删除已证明安全的 started 与 matching result，不能先删结果再留下看似未决的 started。清理与 confirmed 事务统一采用 connection → access credential → refresh credential → started audit 的锁序，锁后重算 digest、重查 matching result；另一用户与普通过期 audit 必须继续按正常 cutoff 处理。
...
preflight 的供应商表面严格只读，但 refresh 本身是可能轮换凭据的外部写效应，因此必须使用 durable fence。`refresh_started`、`refresh_confirmed` 和 `calendar.aad_0019.refresh_terminal` 使用既有 `audit_events`、`task_id=NULL`、`actor_type=system`；metadata 只含 `fence_schema_version`、canonical lowercase `refresh_attempt_id`、`rollout_digest_v1`、`connection_digest`、`authorization_generation`、pre/post `credential_snapshot_digest_v1`、明确允许的 `token_expires_at` 和 `result_code`。本地 credential 存在性、归属和可解密性校验必须在 started 前完成；紧邻 provider 的 started 事务与 provider 后的 credential CAS/confirmed 事务都很短，网络阶段不持有业务事务。已 confirmed 且当前 credential 与事件的 post-digest 匹配时，同一 rollout 从持久 expiry 继续，不再次 refresh；started-without-confirmed 或 needs-attention 对同 authorization generation 的所有 basename 都是硬阻断。仅当既有 OAuth 重新授权使 generation 与当前 digest 都改变，才可建立新 fence；普通 access-token rotation 和显式人工处置都不得成为同 snapshot 的 replay waiver。
...
任一 pair 或 connection 失败时不得继续备份、审计或 migration；数据库仍为 0018。若存在未确认/needs-attention refresh fence，后续同名或不同 basename invocation 都不得再次调用 provider；保持写开关关闭并要求用户通过既有 OAuth 流程重新授权，使授权代际和 `credential_snapshot_digest_v1` 都改变，才能开启新尝试。显式人工处置只能记录绑定原 attempt 的终止/调查结果，不允许同 snapshot 重放；除非连接已有独立持久的不可调用终态，否则该结果也不能让 retention 删除唯一 started 防线。已 confirmed 但尚未发布 artifact 的崩溃恢复必须复用同一 rollout 与持久新 token。其他本地事实失败可在恢复精确目录事实后重新开启完整变更窗口。若数据只能按既有明确用户授权的处置流程删除，应走该流程并重新评估 affected set；禁止 retry waiver、直接 SQL、伪造 credential、access-only 迁移或先迁移再清 marker。
