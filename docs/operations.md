...
唯一自动解锁证明是 append-only content-free `oauth.refresh_credential_replaced`，且只能由原 connection 保持 `connected` 时的渐进重授权/重新同意 callback 产生。发起用于恢复未知 fence 的重授权时，必须在连接锁下读取当前 `start_generation=S`、当前 refresh snapshot 和恰好一个 matching unresolved fence，要求 `S >= fence_generation=F` 且当前 refresh digest 仍精确等于该 fence 的 `pre_refresh_credential_snapshot_digest_v1`；随后复用既有 `set_capabilities_authorizing` 把 generation 递增一次到 `target_generation=T=S+1`，`OAuthAttempt` 仍只持久化现有 target `T`。callback 的 token exchange 保持事务外；保存阶段必须按既有 anti-replay 规则证明 `connection.authorization_generation == OAuthAttempt.target_authorization_generation == T`，由 `T-1` 确定 `S`，并在 `T` 的连接锁下加载旧 refresh credential 与该唯一 unresolved fence。只有供应商明确返回 non-empty 新 refresh token，且受控内存中的 `secrets.compare_digest(old, new)` 为 false，才可在同一短事务以精确 CAS 保存新 access/refresh credential、实际 scopes、能力状态和 replacement proof；callback 后 generation 保持 `post_generation=T`，绝不能再次递增。proof 必须绑定 `proof_schema_version="oauth_refresh_credential_replaced.v1"`、`connection_digest`、原 fence 的 `refresh_attempt_id`、`fence_generation=F`、`start_generation=S`、`target_generation=T`、`post_generation=T`、`old_refresh_credential_snapshot_digest_v1`、`new_refresh_credential_snapshot_digest_v1` 和稳定 `result_code`，并验证 `F <= S`、`T = S + 1`、旧 digest 精确匹配 fence/current CAS、新 digest 精确匹配提交后当前状态；用户由 AuditEvent 的非空 `user_id` 绑定。陈旧或并发 callback 因 target generation 不匹配而 fail closed，credential/scopes/capabilities/proof 任一写入或 commit 失败都整体回滚。callback 没有 refresh token、返回空值或相同 plaintext、connection 已断开、旧 plaintext 不可得、仅更新 access、相同明文重加密或普通 token refresh 均不得生成 proof；`F` 到 `S` 间即使发生其他 generation 变化也不能单独解锁。

365 天审计清理对 refresh fence 有强制例外：unresolved started 没有匹配 confirmed 或可证明安全关闭的 known-terminal 时必须跨 cutoff 保留。retention 只能在显式 `user_id` 范围内解析 content-free metadata；旧 fence 只允许通过 matching confirmed/安全 terminal、原 attempt/F 与 started 精确匹配、`F <= S`、`T=S+1`、`post_generation=T` 且当前 generation/refresh digest 精确匹配 proof post/new 值的 `oauth.refresh_credential_replaced`，或可独立证明的永久 disconnected/数据处置终态解除。任意 full/refresh physical digest、generation、ciphertext、nonce、key version 或 `updated_at` 变化都不能单独清理或解锁；known-terminal 若清理后会失去唯一重放防线，也继续按 unresolved 处理。通用 audit delete 必须排除 started/confirmed/terminal 与 `oauth.refresh_credential_replaced`；专用清理只能在同一事务按 attempt 成组删除已证明安全的 started，以及用于证明安全的 matching result 和存在时的 replacement proof。组内每个待删事件都必须早于 cutoff，等价要求最大 `created_at < cutoff`。旧 started 匹配较新的 result/proof 时不得提前删除，也不能先删结果再留下看似未决的 started。清理与 confirmed 事务统一采用 connection → access credential → refresh credential → started audit 的锁序，锁后重算两个 credential digest、重查 matching result/proof 与当前 generation；另一用户与普通过期 audit 必须继续按正常 cutoff 处理。
...
任一 pair 或 connection 失败时不得继续备份、审计或 migration；数据库仍为 0018。若存在未确认/needs-attention refresh fence，后续同名或不同 basename invocation 都不得再次调用 provider；保持写开关关闭，并只允许原 connection 在仍为 `connected` 时通过既有渐进重授权/重新同意流程取得真正不同的 non-empty refresh token，使 target-generation callback 原子提交 matching `oauth.refresh_credential_replaced` proof 后开启新尝试。**发现未知 fence 时不要先 disconnect**：断开会先删除旧 credential，之后无法比较 old/new plaintext；断开后重连、新建 connection 或映射新 token 都不能生成或替代 proof。callback 未返回 refresh token、返回相同 plaintext、旧 plaintext 不可得、connection 已断开，或仅改变 access/generation/物理密文时仍保持阻断。此时只能走既有明确用户授权的数据处置/删除历史敏感字段流程并重新评估 affected set，或实施经过审查的人工 forward-fix；禁止 fabricated proof、新连接映射、marker skip、retry waiver、直接 SQL、伪造 credential、access-only 迁移或先迁移再清 marker。显式人工调查结果不能授权同 snapshot 重放；除非另有独立持久的不可调用终态，否则也不能让 retention 删除唯一 started 防线。已 confirmed 但尚未发布 artifact 的崩溃恢复必须复用同一 rollout 与持久新 token。其他本地事实失败可在恢复精确目录事实后重新开启完整变更窗口。
