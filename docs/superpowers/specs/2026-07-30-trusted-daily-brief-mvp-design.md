...
规范非负十进制字符串查询参数 `last_event_id`（`0` 或无前导零的数字）。服务端优先 Header，以免自动重连被静态查询游标回退；事件
过期或间隙无法补齐时，客户端重新获取任务快照。REST 与 `task.snapshot` 均携带同一 PostgreSQL
读取视图中的最大 `AuditEvent.id`，公开字段名为规范非负十进制字符串 `event_cursor`。客户端以该游标建立状态、
...
本规格已经由用户复核批准。任何会改变 M1 范围、任务可信模型、数据保留、Google 权限或核心技术栈的修改，都应先更新本规格，再同步修订实施计划。
